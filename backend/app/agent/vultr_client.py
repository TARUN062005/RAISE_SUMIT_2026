...
logger = logging.getLogger("app.vultr")
...
class VultrClient:
    def __init__(self):
        ...
        if not self.api_key:
            raise ValueError("Configuration Error: VULTR_API_KEY is not set in environment")
        if not self.model:
            raise ValueError("Configuration Error: VULTR_MODEL is not set in environment. Automatic model discovery is disabled.")
